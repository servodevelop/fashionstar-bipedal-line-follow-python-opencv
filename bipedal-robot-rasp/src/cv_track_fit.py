...
class TrackFit:
    ...
    def img_preprocess(self, img):
        ...
        # 对二值化图像进行膨胀
        bin_track = cv2.dilate(bin_track, np.ones((3,3), np.uint8), iterations=1)
        ...
        # 将灰度图进行缩放
        gray_small = cv2.resize(gray, dsize=None, fx=self.IMG_SCALE_FACTOR , fy=self.IMG_SCALE_FACTOR)
        ...
        # 限制gray中的最大值
        # 这样做有助于规避强光引起的OSTU阈值的错误
        # 但是这样也会带来不需要的图像梯度变化
        # 例如本来是没有直线的地方, 因为这个操作, 带来的梯度也被认为是直线
        ...
        #     np.ones((3,3), np.uint8) ,iterations=1)
        # 二值化图像膨胀运算
        bin_ostu = cv2.dilate(bin_ostu, np.ones((3,3), np.uint8) ,iterations=1)
        ...
        return has_line, gray_small, bin_track, bin_ostu, bin_line
    
    def visual_img_preprocess(self, has_line, gray_small, bin_track, bin_ostu_close, bin_line):
        '''图像预处理的可视化'''
        # 二值化图像拼接在一起
        canvas = np.hstack((gray_small, bin_track, bin_ostu_close, bin_line))
        # 转换为彩图
        canvas = cv2.cvtColor(canvas, cv2.COLOR_GRAY2BGR)
        # 跟原图拼接在一起
        # canvas = np.hstack((img_small_bgr, canvas))
        # 在画布上添加是否有直线的信息
        # text = 'has line: {}'.format(has_line)
        # cv2.putText(canvas, text=text, org=(0, 0), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 0, 255))
        return canvas
...
def main(argv):
    ...
    while True:
        ...
        # 图像预处理部分
        has_line, gray_small, bin_track, bin_ostu_close, bin_line = tk_curve_fit.img_preprocess(img)
        ...
        # 画布绘制
        canvas = tk_curve_fit.visual_img_preprocess(has_line, gray_small, bin_track, bin_ostu_close, bin_line)
